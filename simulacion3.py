    ...
    def gestion_inventario(self):
        ...
        while True:
            ...
            if self.stock <= self.calcular_punto_reorden():
                ...
                print(Fore.MAGENTA +f"🛒🛒🛒🛒 Solicitud de venta de {cantidad_vendida} unidades de {self.nombre}. Stock actual: {self.stock}"+ Fore.RESET)
                if self.stock > cantidad_vendida:
                    ...
                    if Cliente_decision:
                        feliz= self.stock
                        self.stock = 0
                        print(Fore.GREEN +f"🛒 Cliente ACEPTA compra incompleta. Venta por lo que se tiene en stock. Venta de {feliz} unidades de {self.nombre}. Stock actual: {self.stock} (Tiempo: {self.env.now:.2f})\n"+ Fore.RESET)
                    else:
                        print(Fore.RED +f"🛒 Cliente RECHAZA compra incompleta\n"+ Fore.RESET)
